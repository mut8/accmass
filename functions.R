...
for(s in 0:s.max)
{  
massleft<-x-(mass.Na23 * Na + mass.P31 * p + mass.S32 *s)

Cmax<-as.integer(massleft/12)
Cmin<-as.integer(massleft/50)
...
for(n in 0:ctot){
for(o in 0:ctot){
for(h in floor(ctot/3):((2*ctot+n)+2)) {
...
relationloop<-function(row,  res, RE=2E-5) {
  
#13c
  print("13c")
  res<-findrelation(row, res, mult=1:2, c=-1, c13=1)

#CH2
  print("+CH2")
  res<-findrelation(row, res, c=1, h=2)
...
#H2
  print("H2")
  res<-findrelation(row, res, h=2)

#C2H4O
  print("+C2H4O")
  res<-findrelation(row, res, c=2, h=4, o=1)

#CO2
  print("+CO2")
  res<-findrelation(row, res, c=1, o=2)

#C2H2O
  print("+C2H2O")
  res<-findrelation(row, res, c=2, h=2, o=1)

#O
  print("+O")
  res<-findrelation(row, res, o=1)

#NH
  print("+NH")
  res<-findrelation(row, res, n=1, h=1)
...
#printformula<-function(row) { return (paste("C",c,"H",h,"O",o,"N",n, sep=""))}